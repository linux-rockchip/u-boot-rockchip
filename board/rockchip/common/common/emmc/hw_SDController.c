...
//      All Rights Reserved
//文件名:hw_SDController.c
//描述:RK28 SD/MMC Host Controller driver implement
//作者:hcy
//创建日期:2008-11-08
//更改记录:
//当前版本:1.00
...
hcy 09-07-28  CMMB卡的支持

Revision 1.4  2009/05/07 12:10:12  hcy
hcy 使用小卡座，卡热拔插，很容易出现读不到文件的问题，卡检测改成中断方式

Revision 1.3  2009/04/23 13:59:31  hcy
hcy 支持SDIO命令；中断传输数据的方式可用了；flush cache为DMA读取完毕后再flush
...
/****************************************************************/
//函数名:SDC_EnableSdioInt
//描述:使能SDIO中断，并且注册callback函数
//参数说明:cardId   输入参数  需要操作的卡
//         cb       输入参数  回调函数
//返回值:
//相关全局变量:
...
#if 0
/****************************************************************/
//函数名:SDC_UpdateAhbFreq
//描述:AHB变频时，调用该函数来相应的对SDMMC控制器进行调整，使得
//     卡的工作频率不变
//参数说明:newAhbFreq  输入参数  AHB总线更新后的频率，单位KHz
//返回值:
//相关全局变量:
//注意:
/****************************************************************/
void SDC_UpdateAhbFreq(uint32 newAhbKHz)
{
    ...
    for(; nSDCPort < nSDCPortMax; nSDCPort++)
    {
        ...
        {
            continue;  // 400K设置频率时就已经保证变频不是超过，因此不用再调整
        }

        //剩下的大于400K，这时SDMMC_CLKDIV为0
