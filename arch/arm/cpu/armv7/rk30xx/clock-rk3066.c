...
static const struct pll_clk_set apll_clks[] = {
	//rate, nr, nf, no,	core_div, peri_div, axi_div, hclk_div, pclk_div, ahb2apb_div
	_APLL_SET_CLKS(816000, 1, 34, 1,    1, 8, 3, 2, 4, 2),
	_APLL_SET_CLKS(600000, 1, 25, 1,    1, 4, 3, 2, 4, 2),
};
...
static const struct pll_clk_set gpll_clks[] = {
	...
	_GPLL_SET_CLKS(768000, 1, 32, 1,    4, 2, 4),
	_GPLL_SET_CLKS(600000, 1, 50, 2,    4, 2, 4),
	_GPLL_SET_CLKS(594000, 2, 99, 2,    4, 2, 4),
	_GPLL_SET_CLKS(300000, 1, 25, 2,    2, 1, 2),
...
static void rkclk_pll_wait_lock(enum rk_plls_id pll_id)
{
	...
	uint32 bit = (0x10u << pll_state[pll_id]);
 ...
 * 	when call this function, make sure pll is in slow mode
 */
static void rkclk_cpu_coreclk_set(uint32 pll_src, uint32 core_div, uint32 periph_div)
{
	uint32_t pll_sel = 0, c_div = 0, p_div = 0;
...
static void rkclk_cpu_ahpclk_set(uint32 aclk_div, uint32 hclk_div, uint32 pclk_div, uint32 ahb2apb_div)
{
	...
	g_cruReg->CRU_CLKSEL_CON[1] = (AHB2APB_W_MSK | (AHB2APB_MSK & apb_div))
				...
				| (CORE_ACLK_W_MSK | (CORE_ACLK_MSK & a_div));
}
...
{
	rkclk_cpu_coreclk_set(CPU_SRC_ARM_PLL, clkset->core_div, clkset->periph_div);
...
static uint32 rkclk_get_cpu_aclk_div(void)
{
	...
	div = (con & CORE_CLK_DIV_MSK) + 1;

	con = g_cruReg->CRU_CLKSEL_CON[1];
	switch (con & CORE_ACLK_MSK)
	{
		case CORE_ACLK_11:
			div *= CLK_DIV_1;
			break;
